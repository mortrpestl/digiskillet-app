...
$sql = 'SELECT * FROM TUTORIALS WHERE CATEGORY IN (?, ?, ?, ?, ?) AND TutCat = ? ORDER BY ';

// Check if order is set and assign the appropriate column name
$order = isset($_GET['order']) && ($_GET['order'] == 'ASC' || $_GET['order'] == 'DESC') ? $_GET['order'] : 'ASC';
$categ = isset($_GET['categ']) && in_array($_GET['categ'], ['Title', 'User', 'Date', 'Application']) ? $_GET['categ'] : 'Title';
...
$stmt->bind_param("ssssss", $_GET['a'], $_GET['b'], $_GET['c'], $_GET['d'], $_GET['e'], tut_cat);
$stmt->execute();
